...
COPY IkeMtz.AdventureWorks.Db/bin/Debug*.dacpac /dacpac/
...
RUN /opt/mssql/bin/sqlservr & sleep 30 \ 
    && sqlpackage /Action:Publish /TargetServerName:localhost /TargetUser:SA /TargetPassword:$SA_PASSWORD /SourceFile:/dacpac/IkeMtz.AdventureWorks.Db.dacpac /TargetDatabaseName:AdventureWorksDb /p:BlockOnPossibleDataLoss=false \ 
    && sleep 60 \
